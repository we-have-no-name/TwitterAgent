...
##	c.get_tweets_stream_with_keywords(['the are'])
	c.get_with_ids([432656548536401920])
	
class Collector(TwitterAgent):
	# def __init__(self):
	...
	def get_with_keywords(self, keywords, lang='en', num_per_keyword=20, result_type='mixed'):
		...
		results=self.search_for_tweets_with_keywords(keywords, lang=lang, num_per_keyword=num_per_keyword, result_type=result_type)
		
		#result_type: mixed, recent or popular
		self.store_list_of_objects(results)
		return results
		
	def get_with_ids(self, ids,  batch_size=100):
		"""returns full Tweet objects, specified by the ids parameter, max batch_size is 100"""
		results=self.get_tweets_with_ids(ids, batch_size=batch_size)
		self.store_list_of_objects(results)
		return results
